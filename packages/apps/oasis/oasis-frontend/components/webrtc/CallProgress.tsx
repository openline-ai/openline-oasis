...
const CallProgress: NextPage<CallProgressProps> = (props: CallProgressProps) => {
  ...
  const makeButton = (number: string) => {
    return <button className="btn btn-primary btn-lg m-1" key={"dtmf-" + number}
                   onClick={() => {
                     props.webrtc.current?.sendDtmf(number)
                   }}>{number}</button>
  }
  ...
  let dialpad_rows = []
  for (let i = 0; i < 4; i++) {
    dialpad_rows.push(<div key={"dtmf-row-" + i}
                           className="d-flex flex-row justify-content-center">{dialpad_matrix[i]}</div>)
  }
            ...
            <div style={{position: "relative", width: "100%", height: "100%"}}>
              ...
              <div className='mb-3'>
                <Button onClick={() => toggleMute()} className="mr-2">
                  <FontAwesomeIcon icon={onMute ? faMicrophone : faMicrophoneSlash}
                                   className="mr-2"/> {onMute ? "Unmute" : "Mute"}
                </Button>
                <Button onClick={() => toggleHold()} className="mr-2">
                  <FontAwesomeIcon icon={onHold ? faPlay : faPause}
                                   className="mr-2"/> {onHold ? "Release hold" : "Hold"}
                </Button>
              ...
              {showRefer &&
                <>
                  <div>
                    ...
                    <div>
                      <SuggestionList currentValue={transferDest}
                                      getSuggestions={props.getContactSuggestions}
                                      setCurrentValue={setTransferDest}></SuggestionList>
                    </div>
                  </div>
                </>}
              {inRefer && <div
